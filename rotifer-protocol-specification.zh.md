...
演化闭环里有多少环节可以没有人类介入，不是协议的固定属性，而是**部署者设定的一个刻度盘**。协议把 Agent 自主性分为五级（命名 AL0–AL4，与上文 URAA 架构层的 L0–L4 编号无关）：

| 等级 | 名称 | 含义 |
|---|---|---|
| AL0 | 工具 | 完全由人类驱动，无自主决策 |
| AL1 | 反应式 | 按预设规则响应，有限自主选择 |
| AL2 | 自适应 | 自主演化基因组，**改变本机或公共状态的决策需人类确认** |
| AL3 | 自主式 | 独立运行与演化，人类保留覆盖权 |
| AL4 | 自导向 | 设定自身目标并参与治理，人类保留紧急干预权 |

三条规则让这个刻度盘可信：

- 等级由部署者设定、只能逐级变更；一个 Agent 是否够格升档，由部署者评估。
- 升到自主式（AL3）及以上须显式双重授权；降级任何特权方可单方执行——紧急情况下可直降工具级（AL0）。
- 每次等级变更都留痕：理由与授权一并记录。

参考实现当前运行在**自适应（AL2）**：排名、评测、比较自动进行；改变你机器或公共状态的操作——安装、发布、覆盖、回滚——等待你的批准。**边界是刻度盘，不是墙。**
