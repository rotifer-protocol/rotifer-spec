...
> ⚠️ **稳定性警告：** 跨绑定互操作协议（§10）已在 v0.6.5 版本中通过两个绑定实现（LocalBinding + Web3MockBinding）的部分验证。宿主函数分类（§6）和能力协商协议（§10.5）可能因非 Mock 绑定的实际集成反馈而变更。IR 1.0 将在至少两个*生产级*绑定环境通过跨绑定互操作测试，且完成一轮外部贡献者 RFC 反馈后正式发布。
